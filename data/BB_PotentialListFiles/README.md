This folder will contain the list of candidate for each target v-pin after `runClassification.sh` if `boundingBox` is enabled, with the classification of threshold specified in `runClassification.sh`.
